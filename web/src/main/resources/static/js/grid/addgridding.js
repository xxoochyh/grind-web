...
var provinceList = [{
    ...
    cityList: [{
        name: '西安市',
        areaList: ['市辖区', '新城区', '碑林区', '莲湖区', '灞桥区', '未央区', '雁塔区', '阎良区', '临潼区', '长安区', '鄠邑区', '蓝田县', '周至县', '高陵县']
    }, {
        name: '铜川市',
...
$(function () {
    layui.use(['jquery', 'form', 'layer'], function () {
        ...
        form.on('submit(addgridding)', function (data) {
            //防止都未选择，造成传入空数据
            if (provinceName == null && cityName == null && regionName == null) {
                return false;
            }
            ...
            //获取省的网格数据
            gridData = [];
            var provinceGrid = createObject(provinceName, 1, "防爆监测管理系统");
            var cityGrid = createObject(cityName, 2, provinceName);
            var regionGrid = createObject(regionName, 3, cityName);
            if (provinceGrid != null) {
                gridData.push(provinceGrid);
            }
            if (cityGrid != null) {
                gridData.push(cityGrid);
            }
            if (regionGrid != null) {
                gridData.push(regionGrid);
            }
...
//通过名称获取对象
var createObject = function (name, level, parent) {
    //未选择时为空
    if (name == null) {
        return null;
    }
    ...
    var griddings = $("#gridding").val();
    griddings = JSON.parse(griddings);
    console.log(griddings);
    for (var i = 0; i < griddings.length; i++) {
        if (griddings[i].name === name && griddings[i].level === level && level < 3) {
            return null;
        }
    }
    gridData.name = name;
    gridData.parentName = parent;
...
var loadProvince = function () {
    ...
    for (var i = 0; i < provinceList.length; i++) {
        proHtml += '<option value="' + i + '_' + provinceList[i].name + '_' + provinceList[i].cityList.length + '">' + provinceList[i].name + '</option>';
    }
    //初始化省数据
    ...
    form.on('select(province)', function (data) {
        ...
        provinceName = d[1];
        cityName = provinceList[index].cityList[0].name;
        regionName = provinceList[index].cityList[0].areaList[0];

        var length = d[2];
        console.log(length);
        if (length > 0) {
            loadCity(provinceList[index].cityList);
        } else {
            $form.find('select[name=city]').html('<option value="0_0">请选择</option>').parent().hide();
            $form.find('select[name=region]').html('<option value="0_0">请选择</option>').parent().hide();
        }
    });
}

var loadCity = function (datas) {
    var cityHtml = '';
    var areaHtml = '';
    for (var i = 0; i < datas.length; i++) {
        cityHtml += '<option value="' + i + '_' + datas[i].name + '_' + datas[i].areaList.length + '">' + datas[i].name + '</option>';
    }
    for (var j = 0; j < datas[0].areaList.length; j++) {
        areaHtml += '<option value="' + j + '_' + datas[0].areaList[j] + '_' + datas[0].areaList.length + '">' + datas[0].areaList[j] + '</option>';
    }
    $form.find('select[name=city]').html(cityHtml).parent().show();
    $form.find('select[name=region]').html(areaHtml).parent().show();
    form.render();
    form.on('select(city)', function (data) {
        ...
        //赋值
        cityName = d[1];
        regionName = datas[index].areaList[0];

        console.log(cityName);
        var count = d[2];
        if (count > 0) {
            loadArea(datas[index].areaList);
        }
    })
}

var loadArea = function (datas) {
    var areaHtml = '';
    for (var i = 0; i < datas.length; i++) {
        areaHtml += '<option value="' + i + '_' + datas[i] + '_' + datas.length + '">' + datas[i] + '</option>';
    }
    $form.find('select[name=region]').html(areaHtml).parent().show();
    form.render();

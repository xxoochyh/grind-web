...
const unconfirmedTables = [
    {field: 'alarmId',hide: true, title: '警报id' },
    {field: 'deviceId',  hide: true,title: '设备编号'},
    {field: 'alarmTime', title: '警报时间'},
    {field: 'alarmTypeDesc', title: '警报类型'},
        ...
        success: function (result) {
            ...
            if (result.data == null) {
                ...
                if (data.length > 0) {
                    ...
                    initTable('#unconfirmedTable', '/alarm/getUnconfirmedAlarms/' + data[0].factoryId, [unconfirmedTables]);

                    initTable('#confirmedTable', '/alarm/getConfirmedAlarms/' + data[0].factoryId, [confirmedTables]);
...
var bindClick = function (url) {
    layui.use(['table', 'layer'], function () {
        ...
        //绑定查找监听事件
        $('#searchAlarm').click(function () {
           var q = document.getElementById("keyword").value;
           init(searchUrl+q);//树组件重新加载
        });
        ...
        table.on('tool(unconfirmedTable)', function (obj) {
            ...
            console.log(data);
            if (obj.event === 'edit') {
                createLayer('警报详细信息', 2, '/alarm/alarmDetail/' + data.alarmId, '点击此处返回警报管理页面', false);
            }
        });
    ...
    layui.use(['tree', 'table'], function () {
        ...
        tree.render({
            ...
            click: function (obj) {
                ...
                initTable('#unconfirmedTable',  '/alarm/getUnconfirmedAlarms/' + obj.data.id, [unconfirmedTables]);
                initTable('#confirmedTable', '/alarm/getConfirmedAlarms/' + obj.data.id, [confirmedTables]);
            }
        });
    });
}
